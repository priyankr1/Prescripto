...
const Appointment = () => {
  ...
  const daysOfWeek=['SUN','MON','TUS','WED','THU','FRI','SAT','SUN']
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      currentDate.setDate(today.getDate() + i);  // Fixing this to increment days correctly
      ...
      if (today.getDate() === currentDate.getDate()) {
        ...
        currentDate.setMinutes(0);  // Fixing hours to minutes here
      }
  
      let timeSlots = [];
      while (currentDate < endTime) {
        let formattedTime = currentDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });  // Correcting the method
  ...
  useEffect(() => {
    fetchDocInfo();
  }, [docId]);  // Dependency should be docId, not docInfo or setDocInfo
  
  useEffect(() => {
    if (docInfo) {
      getAvailableSlots();
    }
  }, [docInfo]);  // This effect depends on docInfo
  ...
  return docInfo &&(
    <div>
     ...
     <div className='flex  flex-col sm:flex-row gap-4  '>
      ...
      {/*----------doctor about-------- */}
      <div >
        <p className='flex items-center gap-3 text-sm font-medium text-gray-900 mt-3'>About <img src={assets.info_icon} alt="" /></p>
